...
export default function RFIScreen() {
  ...
  const fetchRfis = async () => {
    try {
      ...
      const token = await SecureStore.getItemAsync("authToken");
      const response = await fetch(
        `https://erp-production-72da01c8e651.herokuapp.com/api/mobile/rfis/${projectId}/rfis`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      const data = await response.json();
  ...
  const fetchContacts = async () => {
    ...
    try {
      const token = await SecureStore.getItemAsync("authToken");
      const response = await fetch(
        `https://erp-production-72da01c8e651.herokuapp.com/api/mobile/contacts/by-account?accountId=${project.accountId._id}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      const data = await response.json();
      if (!response.ok)
      ...
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            ...
            const token = await SecureStore.getItemAsync("authToken");
            const response = await fetch(
              `https://erp-production-72da01c8e651.herokuapp.com/api/mobile/rfis/${projectId}/rfis/${rfiId}`,
              {
                method: "DELETE",
                headers: { Authorization: `Bearer ${token}` },
              }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const url = isEditing
        ? `https://erp-production-72da01c8e651.herokuapp.com/api/mobile/rfis/${projectId}/rfis/${editingId}`
        : `https://erp-production-72da01c8e651.herokuapp.com/api/mobile/rfis/${projectId}/rfis`;
  ...
  const generateRFIPDF = async (rfi) => {
    try {
      ...
      const token = await SecureStore.getItemAsync("authToken");
      const response = await fetch(
        `https://erp-production-72da01c8e651.herokuapp.com/api/mobile/rfis/${projectId}/rfis/${rfi._id}/generate-pdf`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      const body = encodeURIComponent(
        `Please find attached the RFI document for ${project.projectName}.`
      );
      ...
      <View style={tw`px-4 pt-2 pb-4`}>
        ...
        >
          <Text style={tw`text-2xl font-bold text-gray-900`}>
            {project ? project.projectName : "RFIs"}
          </Text>
          <Text style={tw`text-sm text-gray-500 mt-1`}>RFIs</Text>
        </View>
                ...
                rfis.map((rfi) => (
                  ...
                  >
                    ...
                    <View style={tw`flex-row mt-3 flex-wrap`}>
                      ...
                      >
                        <Text
                          style={tw`text-[16px] text-green-600 font-medium`}
                        >
                          Generate PDF
                        </Text>
                      </TouchableOpacity>
          ...
          {isCreating && (
            <>
              ...
              >
                ...
                <Text style={tw`text-[16px] text-gray-700`}>
                  Status:{" "}
                  {newRfi.status.charAt(0).toUpperCase() +
                    newRfi.status.slice(1)}
                </Text>
              </View>
              ...
              >
                ...
                <SelectList
                  ...
                  data={contacts}
                  placeholder={
                    newRfi.customerContacts.length > 0
                      ? newRfi.customerContacts.map((c) => c.value).join(", ")
                      : "Select contacts"
                  }
                  boxStyles={dropdownStyles.boxStyles}
                  inputStyles={dropdownStyles.inputStyles}
                  ...
                  defaultOption={newRfi.customerContacts} // Set default selected options
                />
              </View>
